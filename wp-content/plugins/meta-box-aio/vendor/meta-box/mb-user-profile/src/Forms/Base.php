...
abstract class Base {
	...
	public function __construct( $meta_boxes, $user, $config ) {
		...
		$this->user       = $user;
		$this->config     = $config;
		$this->error      = new Error( 'mbup-form-' . md5( serialize( $config ) ) );
		if ( $this->user ) {
			$this->user->set_error( $this->error );
		}
	}
	...
	public function render() {
		...
		echo '<form class="rwmb-form" method="post" enctype="multipart/form-data" id="' . esc_html( $this->config['form_id'] ) . '">';
	...
	protected function enqueue() {
		...
		wp_enqueue_script( 'mbup', MB_USER_PROFILE_URL . 'assets/user-profile.js', ['jquery', 'password-strength-meter'], '1.4.3', true );
	...
	protected function render_hidden_fields() {
		...
		echo '<input type="hidden" name="action" value="', esc_attr( $this->type ), '">';
